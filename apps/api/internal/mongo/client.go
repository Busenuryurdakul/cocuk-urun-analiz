...
func (c *Client) EnsureIndexes(ctx context.Context) error {
	...
	indexes := []struct {
		...
		{"account_deletion_requests", mongo.IndexModel{Keys: bson.D{{Key: "userId", Value: 1}}}},
		{"account_deletion_requests", mongo.IndexModel{Keys: bson.D{{Key: "codeHash", Value: 1}}, Options: options.Index().SetUnique(true)}},
		{"account_deletion_requests", mongo.IndexModel{Keys: bson.D{{Key: "expiresAt", Value: 1}}, Options: ttl}},
	...
	safetyIndexes := []struct {
		collection string
		model      mongo.IndexModel
	}{
		{"safety_findings", mongo.IndexModel{Keys: bson.D{{Key: "organizationId", Value: 1}, {Key: "analysisRunId", Value: 1}}}},
		{"safety_findings", mongo.IndexModel{Keys: bson.D{{Key: "organizationId", Value: 1}, {Key: "productId", Value: 1}}}},
		{"safety_findings", mongo.IndexModel{Keys: bson.D{{Key: "organizationId", Value: 1}, {Key: "severity", Value: 1}}}},
